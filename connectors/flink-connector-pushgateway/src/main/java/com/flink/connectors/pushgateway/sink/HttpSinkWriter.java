 ...
 *
 * <p>More details on the internals of this sink writer may be found in {@link AsyncSinkWriter} documentation.
 *
...
public class HttpSinkWriter<InputT> extends AsyncSinkWriter<InputT, HttpSinkRequestEntry> {
    ...
    public HttpSinkWriter(
        ...
        int sinkWriterThreadPollSize = Integer.parseInt(properties.getProperty(
                ConnectorConfigConstants.SINK_HTTP_WRITER_THREAD_POOL_SIZE, HTTP_SINK_WRITER_THREAD_POOL_SIZE));
        this.sinkWriterThreadPool = Executors.newFixedThreadPool(sinkWriterThreadPollSize,
                new ExecutorThreadFactory("http-sink-writer-worker", ThreadUtils.LOGGING_EXCEPTION_HANDLER));
    }
